...
When `TRUNK_LOCAL_UPLOAD_DIR` is set to a directory path, the RSpec gem will generate a `trunk_output.bin` file and save it locally, relative to where the test was invoked. This disables the automatic upload to Trunk servers.

**Use case**: This is useful when uploads are failing directly within the context of RSpec, but you still want to use quarantining. By generating the `trunk_output.bin` file locally, you can then upload it separately using the Trunk Analytics CLI.
...
This will create a `trunk_output.bin` file in the `./test-results` directory (relative to where the command was run).

**Uploading with the CLI**:

After generating the `.bin` file, you can upload it using the Trunk Analytics CLI. The CLI supports taking these `.bin` files as input when running the upload command:

```bash
trunk upload --test-reports ./test-results/trunk_output.bin
```
...
#### `TRUNK_LOCAL_UPLOAD_DIR_ALLOW_MULTIPLE` (Experimental)

Set this to `true` (or `1`/`yes`/`on`) to have each local save use a unique filename of the form `trunk_output_{timestamp}.bin`. This prevents parent and child processes from overwriting each other's bundles when they share the same `TRUNK_LOCAL_UPLOAD_DIR`.
