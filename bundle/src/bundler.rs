...
pub async fn parse_internal_bin_and_meta_from_tarball<R: AsyncBufRead>(
    ...
    let versioned_bundle = versioned_bundle
        .ok_or_else(|| anyhow::anyhow!("No {META_FILENAME} file found in the tarball"))?;
...
mod tests {
    ...
    pub async fn test_bundle_meta_is_first_entry() {
        ...
        let tarball_file = tokio::fs::File::open(&bundle_path).await.unwrap();
        let reader = tokio::io::BufReader::new(tarball_file);
