...
use bundle::{
    ...
    BundlerUtil, FileSet, QuarantineBulkTestStatus, QuarantineRunResult, Test, META_VERSION,
};
...
pub async fn run_upload(
    ...
    .await;
    meta.base_props.quarantined_tests = quarantined_tests.to_vec();
...
async fn gather_context(
    ...
    codeowners: Option<CodeOwners>,
    exec_start: Option<SystemTime>,
) -> anyhow::Result<(BundleMeta, Option<BepParseResult>, Option<TempDir>)> {
    let UploadArgs {
        junit_paths,
        #[cfg(target_os = "macos")]
        xcresult_path,
    ...
    let meta = BundleMeta {
        junit_props: BundleMetaJunitProps {
            num_files: file_sets.iter().fold(0, |mut num_files, file_set| {
                num_files += file_set.files.len();
                num_files
            }),
            num_tests: parse_num_tests(&file_sets),
        },
    ...
    Ok((meta, bep_result, junit_path_wrappers_temp_dir))
}
...
fn coalesce_junit_path_wrappers(
    ...
    /* trunk-ignore(clippy/unused_assignments,clippy/unused_mut) */
    let mut junit_path_wrappers_temp_dir = None;
    #[cfg(target_os = "macos")]
    {
        let temp_dir = tempfile::tempdir()?;
        let temp_paths = handle_xcresult(&temp_dir, xcresult_path, repo, org_url_slug)?;
        junit_path_wrappers_temp_dir = Some(temp_dir);
...
async fn extract_exit_code_and_quarantined_tests(
    ...
    let failures = extract_failed_tests(
        &meta_base_props.repo,
        &meta_base_props.org,
        &meta_base_props.file_sets,
    )
    .await;
    let extracted_exit_code = if failures.is_empty() {
        EXIT_SUCCESS
    } else {
        EXIT_FAILURE
    };
    ...
    } = if let Some(quarantine_run_result) = quarantine_run_result {
        quarantine_run_result
    } else {
        run_quarantine(
            api_client,
