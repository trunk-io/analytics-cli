...
pub struct UploadArgs {
    ...
    #[arg(
        long,
        env = constants::TRUNK_REPO_HEAD_SHA_ENV,
    ...
    #[arg(
        long,
        env = constants::TRUNK_CODEOWNERS_PATH_ENV,
    ...
    #[arg(
        long,
        env = constants::TRUNK_ALLOW_EMPTY_TEST_RESULTS_ENV,
    ...
    #[arg(
        long,
        env = constants::TRUNK_DRY_RUN_ENV,
    ...
    #[arg(
        long,
        env = constants::TRUNK_VARIANT_ENV,
    ...
    #[arg(
        long,
        env = constants::TRUNK_TEST_PROCESS_EXIT_CODE_ENV,
    ...
    #[arg(
        long,
        env = constants::TRUNK_REPO_HEAD_AUTHOR_NAME_ENV,
    ...
    #[arg(
        long,
        env = constants::TRUNK_SHOW_FAILURE_MESSAGES_ENV,
...
impl UploadRunResult {
    /// Link to this upload's page, when the run is in test-collection link mode.
    /// Gated on `test_collection_short_id` rather than `base_props.test_collection.short_id`
    /// so an exported-but-blank collection id can't yield a malformed
    /// `/collections//uploads/` link; the payload supplies both halves of the key the
    /// webapp's canonical upload route is addressed by.
