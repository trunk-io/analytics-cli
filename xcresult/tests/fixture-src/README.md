...
regeneration if a bundle stops reproducing it. `nested-and-passing` is checked by
`verify-test-structure.py` instead: its shape is the result tree itself, which no
failure summary describes.
