...
pub fn map_i32_to_test_status(test_status: i32) -> TestStatus {
    ...
    if test_status == TestStatus::RemoteFailure as i32 {
        return TestStatus::RemoteFailure;
    }
    if test_status == TestStatus::FailedToBuild as i32 {
        return TestStatus::FailedToBuild;
    }
    if test_status == TestStatus::ToolHaltedBeforeTesting as i32 {
        return TestStatus::ToolHaltedBeforeTesting;
    }
    TestStatus::NoStatus
}

pub fn map_test_status_to_build_result(test_status: TestStatus) -> TestBuildResult {
    match test_status {
        TestStatus::Passed => TestBuildResult::Success,
        TestStatus::Flaky => TestBuildResult::Success,
        TestStatus::Timeout => TestBuildResult::Failure,
        TestStatus::Failed => TestBuildResult::Failure,
        TestStatus::Incomplete => TestBuildResult::Failure,
        TestStatus::RemoteFailure => TestBuildResult::Failure,
        TestStatus::FailedToBuild => TestBuildResult::Failure,
        TestStatus::ToolHaltedBeforeTesting => TestBuildResult::Failure,
        TestStatus::NoStatus => TestBuildResult::Unspecified,
    }
}
...
pub struct BepTestResult {
    ...
    pub build_status: Option<TestStatus>, // TestStatus from Bazel BEP
                        ...
                        match (payload, id) {
                            ...
                            (Some(Payload::TestResult(test_result)), Some(Id::TestResult(id))) => {
                                    ...
                                    acc.test_results.iter_mut().find(|r| r.label == id.label)
                                {
                                    ...
                                    let new_status = map_i32_to_test_status(test_result.status);
                                    ...
                                    acc.test_results.push(BepTestResult {
                                        ...
                                        build_status: Some(map_i32_to_test_status(
                                            test_result.status,
                                        )),
